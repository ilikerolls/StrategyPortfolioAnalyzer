...
class AnalyzeDataTrades:
    ...
    def combine_strat_stats(self, strat_names: list) -> pd.DataFrame:
        ...
        combined_profit_cum_df = pd.DataFrame(data={'Total Profit': daily_pnl, 'Cum. net profit': cum_sum})
        return combined_profit_cum_df

    def get_daily_dd(self, sel_strats_df: pd.DataFrame) -> float:
        """
    ...
    def get_portfolio_calc_stats(self, strat_names: list) -> float:
        ...
        total_max_dd = self.get_daily_dd(sel_strats_df=combined_stats_df)
        return total_max_dd
